package level_0;

import java.util.Arrays;
import java.util.HashMap;
import java.util.LinkedHashSet;

public class lv0_01_to_00_sample {
	
	static class Solution{	//정답률 순으로 보기 때문에 번호 바뀔 수 있음
		/*
		 	61) 
		*/
		
		
		/*
		 	62) 
		*/
		
		
		/*
		 	63) 
		*/
		
		
		/*
		 	64) 
		*/
		
		
		/*
		 	65) 
		*/
		
		
		/*
		 	66) 
		*/
		
		
		/*
		 	67) 
		*/
		
		 
		/*
		 	68) 
		*/ 
		
		
		/*
		 	69) 
		*/
		
		
		/*
		 	10) 
		*/
		
		
	}

	public static void main(String[] args) {
		Solution sol = new Solution();
		
		//61
		
		//System.out.println("61번 문제 : "+result61);
		
		//62
		
		//System.out.println("62번 문제 : "+result62);
		
		//63
		
		//System.out.println("63번 문제 : "+result63);
		
		//64
		
		//System.out.println("64번 문제 : "+result64);
		
		//65
		
		//System.out.println("65번 문제 : "+result65);
		
		//66
		
		//System.out.println("66번 문제 : "+result66);
		
		//67
		
		//System.out.println("67번 문제 : "+result67);
		
		//68
		
		//System.out.println("68번 문제 : "+Arrays.deepToString(result68));
		
		//69
		
		//System.out.println("69번 문제 : "+Arrays.toString(result69));
		
		//70
		
		//System.out.println("10번 문제 : "+result10);

	}

}
